...
const DynamicLabelContent = inject('store')(
  observer(({ store,region }) => {
    ...
    if(store.labelInfos){
      const labelInfos = JSON.parse(store.labelInfos);

      if(labelInfos){
        const result = constructLabelTree(labelInfos);

        return (
          <>{result.noAssociateOptions&&result.noAssociateOptions.length > 0 &&
...
const DynamicLesionNumber= inject('store')(
  observer(({ store,region }) => {
    ...
    const getLesionOptions = () => {
      const result = localStorage.getItem('lesion-options');
...
const CalcRegionData= inject('store')(
  observer(({ store,region }) => {
    ...
    const calcRegion = ()=>{
      const dpi = 762;
      const origin_width = region.parent.naturalWidth;
      const origin_height = region.parent.naturalHeight;

      if(region.type=='rectangleregion'){
        const heightPercent = region.relativeHeight/100;
        const widthPercent = region.relativeWidth/100;
        const physicalWidth = widthPercent*origin_width*2.54*10/dpi;//单位毫米(mm)
        const physicalHeight = heightPercent*origin_height*2.54*10/dpi;
        const area = (physicalWidth*physicalHeight).toFixed(2);
        //矩形长径(对角线)  短径(短边)
        const longDiameter = Math.sqrt(physicalWidth*physicalWidth+physicalHeight*physicalHeight).toFixed(2);
        const shortDiameter  = (physicalWidth>physicalHeight?physicalHeight:physicalWidth).toFixed(2);

        return { area,longDiameter,shortDiameter };
      }else if(region.type=='ellipseregion'){//椭圆
        const radiusXPercent = region.relativeRadiusX/100;
        const radiusYPercent = region.relativeRadiusY/100;
        const physicalRx = radiusXPercent*origin_width*2.54*10/dpi;//单位毫米(mm)
        const physicalRy = radiusYPercent*origin_height*2.54*10/dpi;
        const area = (Math.PI*physicalRx*physicalRy).toFixed(2);
        const longDiameter = ((physicalRx>physicalRy?physicalRx:physicalRy)*2).toFixed(2);
        const shortDiameter = ((physicalRx>physicalRy?physicalRy:physicalRx)*2).toFixed(2);
